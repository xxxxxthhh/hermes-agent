        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                ...
                if getattr(message.author, "bot", False):
                    ...
                    _is_dm_channel = isinstance(message.channel, discord.DMChannel)
                    _raw_self_mentioned = self._self_is_raw_mentioned(message)

                    # Controlled bot-to-bot collaboration rule:
                    # - DMs may pass with DISCORD_ALLOW_BOTS=all.
                    # - Shared channels/threads require a literal inline
                    #   <@this_bot> token even for allow_bots=all, so reply
                    #   chips / ambient bot chatter cannot wake us.
                    # - allow_bots=mentions always requires an explicit self
                    #   mention; raw-content mentions are accepted even if
                    #   Discord did not populate message.mentions.
                    if not _is_dm_channel and not _raw_self_mentioned:
                        return
                    if allow_bots == "mentions" and not self._self_is_explicitly_mentioned(message):
                        return
                    if self._discord_bots_require_inline_mention() and not _raw_self_mentioned:
                        return

                    # Anti-loop: drop bot reply messages unless they explicitly
                    # include this bot as an inline mention in the message body.
                    # Discord reply-pings can add us to message.mentions without
                    # the author actually addressing us, which fuels loops.
                    if message.type == discord.MessageType.reply and not _raw_self_mentioned:
                        logger.debug(
